      ...
      {categories.map((category) => (
        ...
        >
          ...
          <div className="flex items-center space-x-3 mb-4">
            <div className={cn('w-12 h-12 rounded-lg flex items-center justify-center text-2xl', category.color)}>
              {category.icon}
            </div>
            <div>
              <h3 className="text-lg font-semibold text-txt">{category.name}</h3>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <ul className="space-y-1">
              {category.examples.slice(0, 2).map((example, index) => (
                <li key={index} className="text-xs text-txt-secondary flex items-start">
