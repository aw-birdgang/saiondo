...
const SettingsCard: React.FC<SettingsCardProps> = ({ option, onClick }) => (
  ...
  >
    {/* 배경 그라데이션 */}
    <div
      className={`absolute inset-0 bg-gradient-to-br opacity-5 group-hover:opacity-10 transition-opacity duration-300 ${option.bgColor}`}
    />

    <div className='relative p-6'>
      <div className='flex items-center gap-4'>
        <div
          className={`w-12 h-12 rounded-xl bg-gradient-to-br flex items-center justify-center text-white text-xl shadow-lg transition-all duration-200 group-hover:scale-110 ${option.color}`}
        >
          {option.icon}
        </div>
