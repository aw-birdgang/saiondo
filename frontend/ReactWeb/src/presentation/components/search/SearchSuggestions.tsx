          ...
          <div className="space-y-2">
            {suggestions.length > 0 ? (
              suggestions.map((suggestion, index) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    <svg className="w-4 h-4 text-txt-secondary group-hover:text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                    <span className="text-txt group-hover:text-primary">{suggestion}</span>
                  </div>
                </button>
              ))
            ) : (
              <div className="text-center py-8">
                ...
                <p className="text-txt-secondary">검색어를 입력하면 추천 검색어가 나타납니다.</p>
              </div>
            )}
          </div>
        </div>
              ...
              recentSearches.map((search, index) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-txt group-hover:text-primary">{search}</span>
                  </div>
                </button>
              ))
            ) : (
