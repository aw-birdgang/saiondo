...
import { Card, CardContent, CardHeader, CardTitle, Button, Input } from '../../common';
            ...
            <div className="flex items-center">
              <label className="flex items-center">
                <input
                  ...
                  onChange={(e) => setFilters(prev => ({ ...prev, hasSocialLinks: e.target.checked }))}
                  className="mr-2"
                />
                <span className="text-sm text-gray-700">{t('has_social_links')}</span>
              </label>
            </div>
          </div>
        </form>
...
const ProfileSearchResult: React.FC<ProfileSearchResultProps> = ({ profile, onClick }) => {
  const { t } = useTranslation();
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center space-x-4 mt-1 text-xs text-gray-500">
          ...
          <span>{profile.stats.followersCount} {t('followers')}</span>
          <span>{profile.stats.postsCount} {t('posts')}</span>
        </div>
      </div>
