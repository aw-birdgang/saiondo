...
const ProfileAnalytics: React.FC<ProfileAnalyticsProps> = ({
  ...
  const mockAnalytics: AnalyticsData = {
    ...
    engagementRate: 8.7,
    topPosts: [
      { id: '1', title: '첫 번째 게시물', views: 1200, likes: 89, comments: 23 },
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-900">{t('profile_analytics')}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">{t('followers_growth')}</p>
                <p className="text-2xl font-bold text-green-600">+{analytics.followersGrowth}%</p>
              </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">{t('views_growth')}</p>
                <p className="text-2xl font-bold text-blue-600">+{analytics.viewsGrowth}%</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{analytics.engagementRate}%</p>
              </div>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {post.views} {t('views')} • {post.likes} {t('likes')} • {post.comments} {t('comments')}
                    </p>
                  </div>
                </div>
          ...
          <div className="h-64 flex items-end space-x-2">
            {analytics.activity.map((day, index) => (
              <div key={day.date} className="flex-1 flex flex-col items-center space-y-2">
                <div className="w-full bg-blue-100 rounded-t" style={{ height: `${(day.followers / 200) * 100}%` }}>
