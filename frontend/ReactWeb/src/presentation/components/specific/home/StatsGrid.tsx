  ...
  return (
    ...
    >
      ...
      <div className='absolute inset-0'>
        <div
          className={cn(
            'absolute inset-0 transform rotate-12 scale-150 opacity-10',
            colors.bg.replace('bg-gradient-to-br', 'bg-gradient-to-r')
          )}
        ></div>
      ...
      <CardContent className='relative p-6 lg:p-8'>
        <div className='flex items-center justify-between'>
          <div className='flex-1 space-y-4'>
            <div className='flex items-center justify-between'>
              <p
                className={cn(
                  'text-sm font-bold transition-colors duration-300 uppercase tracking-wider',
                  colors.text
                )}
              >
                {title}
            ...
            <div
              className={cn(
                'text-3xl lg:text-4xl font-bold transition-colors duration-300',
                colors.text
              )}
            >
              <AnimatedCounter
        ...
        <div className='absolute top-0 right-0 w-20 h-20 lg:w-24 lg:h-24 opacity-10'>
          <div
            className={cn('w-full h-full rounded-full blur-xl', colors.icon)}
          ></div>
        </div>
      </CardContent>
    </Card>
...
const StatsGrid: React.FC<StatsGridProps> = ({ stats, className }) => {
  const colors: Array<'red' | 'blue' | 'green' | 'purple' | 'orange'> = [
    'blue',
    'green',
    'purple',
    'orange',
