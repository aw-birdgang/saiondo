...
const ChatInput: React.FC<ChatInputProps> = ({ 
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      toast.error(t("chat.file_too_large") || "파일 크기는 10MB 이하여야 합니다.");
      return;
    }
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.error(t("chat.file_type_not_supported") || "지원하지 않는 파일 형식입니다.");
      return;
    }
    ...
    if (onSendFile) {
      ...
      toast.error(t("chat.file_upload_not_supported") || "파일 업로드가 지원되지 않습니다.");
    }
