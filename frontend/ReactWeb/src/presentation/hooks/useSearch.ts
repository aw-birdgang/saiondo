...
export const useSearch = (searchUseCase: ISearchUseCase) => {
  ...
  const performSearch = useCallback(async (query: string, page: number = 1) => {
    ...
    try {
      const request: SearchRequest = {
        query,
        filters: state.selectedFilters.includes('all') ? [] : state.selectedFilters,
        page,
        limit: 10
      };
      ...
      const updatedFilters = SEARCH_FILTERS.map(filter => ({
        ...
        count: filter.type === 'all' 
          ...
          : response.results.filter(result => result.type === filter.type).length
      }));
  ...
  }, [state.hasMore, state.isSearching, state.query, state.currentPage, performSearch]);
