...
export const useMessageStore = create<MessageState>()((set, get) => ({
  ...
  addReaction: (channelId, messageId, reaction: MessageReaction) =>
    set(state => ({
      messages: {
        ...state.messages,
        [channelId]: (state.messages[channelId] || []).map(message =>
          message.id === messageId
            ? {
                ...message,
                reactions: [...((message as any).reactions || []), reaction],
              }
            : message
        ),
      },
      ...
      set(state => ({
        messages: {
          ...
          [channelId]:
            offset === 0
              ? messageData
              : [...(state.messages[channelId] || []), ...messageData],
        },
        hasMore: {
          ...state.hasMore,
  ...
  editMessage: async (messageId: string, newContent: string) => {
    try {
      ...
      const updatedMessage = await messageRepository.editContent(
        messageId,
        newContent
      );
