...
export class AnalyticsController extends BaseController {
  ...
  private async checkAnalyticsPermission(
    ...
    try {
      ...
      this.logger.warn(`Failed to check analytics permission for user ${userId}:`, error);
  ...
  private async getCachedAnalytics<T>(cacheKey: string): Promise<T | null> {
    try {
      ...
      this.logger.warn(`Failed to get cached analytics for ${cacheKey}:`, error);
      return null;
    }
  }
  ...
  async getUserActivityAnalytics(
    ...
    return this.executeWithTracking(
      ...
      async () => {
        ...
        const cachedResult = await this.getCachedAnalytics<ActivityAnalytics>(cacheKey);
        ...
        // 사용자 활동 로그 조회
        const userActivities = await this.userActivityLogUseCase?.getUserActivityLogs(userId) || [];
        ...
        // 결과 캐싱
        await this.cacheAnalytics(cacheKey, analytics, this.USER_ANALYTICS_CACHE_TTL);
  ...
  async getChannelActivityAnalytics(
    ...
    return this.executeWithTracking(
      ...
      async () => {
        ...
        const cachedResult = await this.getCachedAnalytics<ChannelActivityAnalytics>(cacheKey);
        ...
        // 채널 활동 로그 조회
        const channelActivities = await this.userActivityLogUseCase?.getChannelActivityLogs(channelId) || [];
        
        // 시간 범위에 따른 필터링
        const filteredActivities = this.filterActivitiesByTimeRange(channelActivities, timeRange);
        ...
        // 결과 캐싱
        await this.cacheAnalytics(cacheKey, analytics, this.CHANNEL_ANALYTICS_CACHE_TTL);
  ...
  async getSystemAnalytics(userId: string): Promise<SystemAnalytics> {
    return this.executeWithTracking(
      ...
      async () => {
        ...
        const cachedResult = await this.getCachedAnalytics<SystemAnalytics>(cacheKey);
        ...
        // 결과 캐싱
        await this.cacheAnalytics(cacheKey, systemStats, this.SYSTEM_ANALYTICS_CACHE_TTL);
   ...
   * 시간 범위에 따른 활동 필터링
   */
  private filterActivitiesByTimeRange(activities: any[], timeRange: TimeRange): any[] {
    const now = new Date();
  ...
  private findMostActivePeriod(activities: any[]): { hour: number; count: number; percentage: number } {
    const hourlyStats = this.groupActivitiesByHour(activities);
    const maxHour = Object.entries(hourlyStats).reduce((max, [hour, count]) => 
    ...
    return {
      ...
      count: maxHour.count,
      percentage: activities.length > 0 ? (maxHour.count / activities.length) * 100 : 0
    };
  }
  ...
  private calculateEngagementScore(activities: any[]): number {
    ...
    const diversityScore = Math.min(Object.keys(this.groupActivitiesByType(activities)).length / 5, 1) * 30; // 최대 30점
  ...
  private calculateRecencyScore(activities: any[]): number {
    ...
    const now = new Date();
    const latestActivity = new Date(Math.max(...activities.map(a => new Date(a.timestamp).getTime())));
    const hoursSinceLastActivity = (now.getTime() - latestActivity.getTime()) / (1000 * 60 * 60);
    
    // 24시간 이내면 1점, 그 이후로는 감소
    return Math.max(0, 1 - (hoursSinceLastActivity / 24));
  }

  private analyzeTrends(activities: any[], timeRange: string): { trend: 'increasing' | 'decreasing' | 'stable'; change: number } {
  ...
  private analyzeMessageStats(activities: any[]): { totalMessages: number; averageLength: number; messagesWithFiles: number; mostActiveHour: any } {
    const messageActivities = activities.filter(a => a.action === 'MESSAGE_SEND');
    
    return {
      totalMessages: messageActivities.length,
      averageLength: messageActivities.length > 0 
        ? messageActivities.reduce((sum, a) => sum + (a.details?.contentLength || 0), 0) / messageActivities.length 
        : 0,
      messagesWithFiles: messageActivities.filter(a => a.details?.hasAttachments).length,
      mostActiveHour: this.findMostActivePeriod(messageActivities)
    };
  }

  private calculateUserEngagement(activities: any[]): { totalUsers: number; averageActivitiesPerUser: number; mostActiveUser: { user: string; count: number } } {
    const userStats = this.groupActivitiesByUser(activities);
    const users = Object.keys(userStats);
    
    return {
      totalUsers: users.length,
      averageActivitiesPerUser: users.length > 0 ? activities.length / users.length : 0,
      mostActiveUser: Object.entries(userStats).reduce((max, [user, count]) => 
        count > max.count ? { user, count } : max, { user: '', count: 0 }
      )
    };
  }

  private findPeakActivityTimes(activities: any[]): Array<{ hour: number; count: number }> {
    const hourlyStats = this.groupActivitiesByHour(activities);
  ...
  private analyzePreferredChannels(activities: any[]): string[] {
    ...
    const channelStats = channelActivities.reduce((acc, activity) => {
  ...
  private analyzeActivityPattern(activities: any[]): string {
    const hourlyStats = this.groupActivitiesByHour(activities);
    const morning = [6, 7, 8, 9, 10, 11].reduce((sum, hour) => sum + (hourlyStats[hour] || 0), 0);
    const afternoon = [12, 13, 14, 15, 16, 17].reduce((sum, hour) => sum + (hourlyStats[hour] || 0), 0);
    const evening = [18, 19, 20, 21, 22, 23].reduce((sum, hour) => sum + (hourlyStats[hour] || 0), 0);
    const night = [0, 1, 2, 3, 4, 5].reduce((sum, hour) => sum + (hourlyStats[hour] || 0), 0);
  ...
  private calculateChurnRisk(activities: any[]): number {
    ...
    return Math.round((1 - recencyScore) * 0.6 + (1 - engagementScore / 100) * 0.4) * 100;
  }
  ...
  private getPerformanceMetrics(): { responseTime: number; errorRate: number; uptime: number } {
    return {
