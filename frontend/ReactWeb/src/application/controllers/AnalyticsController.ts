...
export class AnalyticsController extends BaseController {
  ...
  async getUserActivityAnalytics(
    ...
    return this.executeWithTracking(
      ...
      async () => {
        ...
        const cachedResult = await this.cacheUseCase.get(cacheKey);
        ...
        // 결과 캐싱 (5분)
        await this.cacheUseCase.set(cacheKey, analytics, 300);
  ...
  async getChannelActivityAnalytics(
    ...
    return this.executeWithTracking(
      ...
      async () => {
        ...
        const filteredActivities = channelActivities.filter(
          activity => new Date(activity.timestamp) >= cutoffTime
        );
        ...
        const analytics = {
          ...
          trends: this.analyzeChannelTrends(filteredActivities, timeRange)
        };

        // 결과 캐싱 (10분)
        await this.cacheUseCase.set(cacheKey, analytics, 600);
   ...
   * 시스템 전체 통계
   */
  async getSystemAnalytics(userId: string): Promise<any> {
    return this.executeWithTracking(
      'getSystemAnalytics',
      { userId },
      async () => {
        ...
        // 결과 캐싱 (30분)
        await this.cacheUseCase.set(cacheKey, systemStats, 1800);
   ...
   * 실시간 대시보드 데이터
   */
  async getRealTimeDashboard(userId: string): Promise<any> {
    return this.executeWithTracking(
  ...
  private findMostActivePeriod(activities: any[]): any {
    ...
    return {
      ...
      count: maxHour.count,
      percentage: (maxHour.count / activities.length) * 100
    };
  }
  ...
  private analyzeTrends(activities: any[], timeRange: string): any {
  ...
  private analyzeMessageStats(activities: any[]): any {
    const messageActivities = activities.filter(a => a.action === 'MESSAGE_SEND');
    
    return {
      totalMessages: messageActivities.length,
      averageLength: messageActivities.reduce((sum, a) => sum + (a.details?.contentLength || 0), 0) / messageActivities.length || 0,
  ...
  private calculateUserEngagement(activities: any[]): any {
    const userStats = this.groupActivitiesByUser(activities);
    const users = Object.keys(userStats);
    
    return {
      totalUsers: users.length,
      averageActivitiesPerUser: activities.length / users.length || 0,
  ...
  private findPeakActivityTimes(activities: any[]): any[] {
    const hourlyStats = this.groupActivitiesByHour(activities);
  ...
  private analyzeChannelTrends(activities: any[], timeRange: string): any {
    return this.analyzeTrends(activities, timeRange);
  }
