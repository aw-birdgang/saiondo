...
export class InviteToChannelUseCase {
  ...
  async execute(request: InviteToChannelRequest): Promise<InviteToChannelResponse> {
    try {
      ...
      const isInviterMember = await this.channelService.isMember(request.channelId, request.inviterId);
      if (!isInviterMember) {
      ...
      for (const inviteeId of request.inviteeIds) {
        try {
          ...
          const isAlreadyMember = await this.channelService.isMember(request.channelId, inviteeId);
          if (isAlreadyMember) {
