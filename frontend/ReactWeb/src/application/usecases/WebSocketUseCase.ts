...
export class WebSocketUseCase
  implements IUseCase<ConnectWebSocketRequest, ConnectWebSocketResponse>
{
  ...
  async sendWebSocketMessage(
    ...
    const serviceRequest = {
      ...
      message:
        typeof request.message.data === 'string'
          ? request.message.data
          : JSON.stringify(request.message.data),
  ...
  async sendTypingIndicator(
    request: SendTypingIndicatorRequest
  ...
  async getConnectionInfo(
    ...
    const connection =
      connections.find(conn => conn.userId === request.userId) || null;
    return { connection };
  }

  async getActiveConnections(
    request: GetActiveConnectionsRequest
  ): Promise<GetActiveConnectionsResponse> {
    const connections = this.webSocketService.getActiveConnections();
    return { connections };
  }

  async getWebSocketStats(
    request: GetWebSocketStatsRequest
  ): Promise<GetWebSocketStatsResponse> {
