...
export class ChannelService {
  async fetchAllChannels(): Promise<Channel[]> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '채널 목록 조회 실패';
  ...
  async fetchChannelById(channelId: string): Promise<Channel> {
    try {
      const response = await apiClient.get<Channel>(ENDPOINTS.CHANNEL_BY_ID(channelId));
      return response;
    } catch (error: any) {
      const message = error.response?.data?.message || error.message || '채널 조회 실패';
  ...
  async createChannel(userId: string): Promise<Channel> {
    try {
      const response = await apiClient.post<Channel>(ENDPOINTS.CHANNELS, { userId });
      return response;
    } catch (error: any) {
      const message = error.response?.data?.message || error.message || '채널 생성 실패';
  ...
  async joinByInvite(inviteCode: string, userId: string): Promise<Channel> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '초대코드로 채널 가입 실패';
  ...
  async leaveChannel(userId: string): Promise<void> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '채널 나가기 실패';
  ...
  async addMember(channelId: string, userId: string): Promise<void> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '멤버 추가 실패';
      throw new Error(`멤버 추가 실패: ${message}`);
    }
  }

  async createInviteCode(channelId: string, userId: string): Promise<{ inviteCode: string }> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '초대 코드 생성 실패';
  ...
  async acceptInvitation(channelId: string, userId: string): Promise<void> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '초대 수락 실패';
  ...
  async rejectInvitation(channelId: string, userId: string): Promise<void> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '초대 거절 실패';
  ...
  async deleteChannel(channelId: string): Promise<void> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '채널 삭제 실패';
  ...
  async isMember(channelId: string, userId: string): Promise<boolean> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '멤버십 확인 실패';
  ...
  async cleanupEmptyChannels(): Promise<void> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '멤버 없는 채널 정리 실패';
  ...
  async createInvitation(
    ...
    try {
      ...
      const message = error.response?.data?.message || error.message || '초대장 생성 실패';
      throw new Error(`초대장 생성 실패: ${message}`);
    }
  }

  async hasPendingInvitation(channelId: string, inviteeId: string): Promise<boolean> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '초대장 상태 확인 실패';
  ...
  async fetchInvitationsForUser(userId: string): Promise<ChannelInvitation[]> {
    try {
      ...
      const message = error.response?.data?.message || error.message || '초대장 목록 조회 실패';
      throw new Error(`초대장 목록 조회 실패: ${message}`);
    }
  }

  async respondInvitation(invitationId: string, response: 'ACCEPT' | 'REJECT'): Promise<void> {
    try {
      await apiClient.post(ENDPOINTS.RESPOND_INVITATION(invitationId), {
        response
      });
    } catch (error: any) {
      const message = error.response?.data?.message || error.message || '초대장 응답 실패';
