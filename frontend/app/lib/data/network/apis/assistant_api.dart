...
class AssistantApi {
  ...
  Future<List<Assistant>> fetchAssistants(String userId) async {
    final response =
        await _dioClient.dio.get(Endpoints.assistantsByUser(userId));
  ...
  Future<Assistant> fetchAssistantById(String assistantId) async {
    final response =
        await _dioClient.dio.get(Endpoints.assistantById(assistantId));
    return Assistant.fromJson(response.data);
  }

  Future<Assistant> createAssistant(AssistantCreateRequest request) async {
    final response =
        await _dioClient.dio.post(Endpoints.assistants, data: request.toJson());
    return Assistant.fromJson(response.data);
  }

  Future<Assistant> updateAssistant(
      String assistantId, AssistantUpdateRequest request) async {
    final response = await _dioClient.dio
        .put(Endpoints.assistantById(assistantId), data: request.toJson());
    return Assistant.fromJson(response.data);
  }
