import {MessageSender, PrismaClient, ProfileSource, RelationshipStatus} from '@prisma/client';
...
async function main() {
  ...
  // 3. 질문 생성 (최적화: Promise.all로 병렬 처리)
  ...
  // 4. CategoryCode 생성 (배열+map으로 최적화)
  const categoryCodes = [
  ...
  // 10. 이벤트(Event) 생성 (createMany 한 번에)
