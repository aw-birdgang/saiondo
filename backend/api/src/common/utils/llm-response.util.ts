...
export class LLMResponseUtil {
  ...
  static parseResponse(response: any): any {
    try {
      ...
      if (typeof response === 'string') {
        ...
        logger.debug(
          `[LLMResponseUtil] 문자열 파싱 성공: ${JSON.stringify(parsed).substring(0, 200)}...`,
        );

        return parsed;
      } else if (response && typeof response.response === 'string') {
        logger.debug(
          `[LLMResponseUtil] response.response 문자열 파싱: 길이=${response.response.length}`,
        );
        const parsed = JSON.parse(response.response);

        logger.debug(
          `[LLMResponseUtil] response.response 파싱 성공: ${JSON.stringify(parsed).substring(0, 200)}...`,
        );
        ...
        if (response.response && typeof response.response === 'object') {
          logger.debug(
            `[LLMResponseUtil] 중첩된 response 객체 처리: ${JSON.stringify(response.response).substring(0, 200)}...`,
          );

          return response.response;
        }
        logger.debug(
          `[LLMResponseUtil] 객체 응답 그대로 반환: ${JSON.stringify(response).substring(0, 200)}...`,
        );
      ...
      logger.error(
        `[LLMResponseUtil] JSON 파싱 실패: response=${JSON.stringify(response).substring(0, 200)}...`,
        error,
      );
  ...
  static getFields(response: any, fields: Record<string, any>): Record<string, any> {
    try {
      ...
      logger.error(
        `[LLMResponseUtil] 다중 필드 추출 실패: fields=${JSON.stringify(fields)}`,
        error,
      );
