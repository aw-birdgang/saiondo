...
export class ChatService {
  ...
  async sendToLLM(
    ...
    const userChat = await this.saveChatMessage(userId, assistantId, channelId, message, MessageSender.USER);
    const llmResponse = await this.llmService.getFeedback(prompt, assistantId);
    const aiChat = await this.saveChatMessage(userId, assistantId, channelId, llmResponse, MessageSender.AI);
...
// chatService
  ...
  private async getPersonaSummary(userId: string): Promise<string> {
    const personaList: PersonaProfile[] = await this.personaProfileService.getPersonaByUserId(userId);
  ...
  private buildPrompt(questioner: User | null, partner: User, personaSummary: string, message: string): string {
    return `
