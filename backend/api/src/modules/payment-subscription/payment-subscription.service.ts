...
export class PaymentSubscriptionService {
  ...
  async buyPoint(userId: string, dto: BuyPointDto) {
    ...
    if (!product?.isActive) throw new NotFoundException('상품 없음');
    // 실제 결제 연동은 생략(성공 가정)
  ...
  async subscribe(userId: string, dto: SubscribeDto) {
    ...
    // 실제 결제 연동은 생략(성공 가정)
    const now = new Date();
    const periodInMs = selected.periodDays * 24 * 60 * 60 * 1000;
    const expiredAt = new Date(now.getTime() + periodInMs);
  ...
  private verifyGoogleReceipt(_receipt: string) {
    // 실제 Google API 엔드포인트 사용
    // OAuth2 인증 필요 (생략)
    // 예시용
    const isValid = true;
    const thirtyDaysInMs = 30 * 24 * 60 * 60 * 1000;
    const expiresAt = new Date(Date.now() + thirtyDaysInMs); // 30일 후
