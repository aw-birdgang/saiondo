...
def batch_couple_analysis(request: CoupleAnalysisBatchRequest, background_tasks: BackgroundTasks):
    try:
        results = []
        success_count = 0
        ...
        for couple_data in request.couples:
            try:
                ...
                if cached_result:
                    ...
                    result = enhanced_couple_analysis_service.analyze_couple(user_data, partner_data)
                ...
                # 실패한 경우 기본 결과 추가
                fallback_result = enhanced_couple_analysis_service._get_fallback_analysis()
