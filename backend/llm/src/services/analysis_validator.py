...
class AnalysisValidator:
    ...
    def validate_couple_data(self, user_data: Dict, partner_data: Dict) -> None:
        ...
        if 'mbti' in user_data and user_data['mbti'] not in valid_mbti_types + ['UNKNOWN']:
            errors.append(ValidationError(
                field="user_data.mbti",
                message=f"유효하지 않은 MBTI 타입입니다. 유효한 타입: {', '.join(valid_mbti_types)}",
                code="INVALID_MBTI_TYPE"
            ))
        
        if 'mbti' in partner_data and partner_data['mbti'] not in valid_mbti_types + ['UNKNOWN']:
            errors.append(ValidationError(
